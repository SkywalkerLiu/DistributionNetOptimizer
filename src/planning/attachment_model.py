...
def build_attachment_options(
    *,
    ...
    for row in users.itertuples():
        ...
        if not group_id:
            raise ValueError(f"No service group was generated for user {user_id}.")
        attach_node_id = corridor.service_group_attach_node.get(group_id, "")
        if not attach_node_id:
            raise ValueError(
                f"No service-group attach node was generated for service group {group_id}."
            )
        if attach_node_id not in corridor.nodes:
            raise ValueError(
                f"Service-group attach node {attach_node_id} for service group {group_id} is missing."
            )

        node = corridor.nodes[attach_node_id]
        ...
        if not segment_is_feasible(
            ...
            reasons.append(f"no_corridor_to_user_{user_id}")
