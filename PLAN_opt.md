...
低压网络采用充分共享的线路结构：用户作为终端负荷，通过接户线挂接到已有杆塔或局部新增接入杆塔，同一条低压线路段可同时承载多个用户的下游负荷。ABC 三相平衡的对象是台区出口及各共享线路分段累计的 `A/B/C` 相负荷平衡，而不是为不同用户分别建设独立线路。

## Optimization Objective

核心目标是在满足容量、压降、禁区、档距、不交叉和三维地形约束的前提下，最小化建设总成本，并在尽可能复用共享低压线路的同时，使台区出口及各共享线路分段三相累计负载基本平衡。
...
`construction_cost` 包括配变成本、杆塔成本、线路长度成本、导线成本、接户线成本和地形施工附加成本。`phase_unbalance_penalty` 是正式优化目标项，不是后处理，同时约束台区总三相负荷和各共享低压线路分段下游累计三相负荷。该项惩罚针对线路段级别的 `A/B/C` 负荷平衡，而不是对用户逐户独立考核。
...
- 低压侧必须为以配变为根的无环径向网络。
- 低压网络以共享线路为主体，用户为终端负荷节点，不作为其他用户的中继节点；多个用户允许通过同一杆塔或同一线路段接入。
- 线路不得穿越树林、水区、人工禁区和不可通行区域。
- 杆塔、配变和接户点不得落入硬禁区或不可建设区域。
- 平面投影中线路不得出现无节点交叉，允许共享端点、共杆同路径或同一路径承载 `ABCN`。
- 相邻杆塔之间的平面档距不得超过 50 m。
- 接户线平面档距不宜大于 25 m；接户线仅用于用户终端从共享线路杆塔接入，不作为独立主干重复延伸。
- 主干线路必须按同一路径承载 `A/B/C/N` 四线，不能为了相别分配拆成不同路径。
- 除非受禁区、压降或施工条件限制，低压网络应优先复用已建共享线路和杆塔，不为不同用户重复铺设平行的同向线路。
- 每个单相用户必须由优化算法写入 `assigned_phase in {A, B, C}`。
- 每条低压线路段必须汇总其全部下游共享用户的 `A/B/C` 负荷、零线电流估算、三相不平衡率和压降。
- 三相平衡约束的考核对象为台区出口及每条共享低压线路段，不要求每个用户独立形成三相对称。
...
`users` 图层新增或预留字段：`load_kw`、`power_factor`、`phase_type`、`assigned_phase`、`apparent_kva`、`elev_m`、`attached_pole_id`、`service_line_id`、`service_drop_length_m`、`connected_node_id`、`voltage_drop_pct`。
...
`planned_lines.line_type` 推荐区分 `hv_backbone`、`lv_main`、`lv_branch`、`service_drop`，其中 `lv_main/lv_branch` 为可共享线路段，`service_drop` 仅表示用户终端接入。

`planned_lines` 图层字段：`line_id`、`line_type`、`from_node`、`to_node`、`parent_line_id`、`phase_set`、`service_phase`、`horizontal_length_m`、`length_3d_m`、`dz_m`、`slope_deg`、`cost`、`downstream_user_count`、`load_a_kva`、`load_b_kva`、`load_c_kva`、`segment_unbalance_ratio`、`neutral_current_a`、`voltage_drop_pct`、`is_violation`。
...
6. 用户接入与局部补杆：对每个用户在最大接户线平面距离 25 m 内优先搜索现有共享杆塔或分支接入点；允许多个用户挂接同一杆塔。若现有杆塔无法满足，才允许在局部路径走廊内增设接入杆塔或短支线；若仍不可行，触发局部路径重规划或输出诊断。
7. 低压径向网络生成：将各用户需求投影到共享线路候选图上，在满足放射式、无环、共享主干优先和总成本最小的原则下，生成以配变为根、可被多个用户共同复用的低压主干-分支网络。用户作为终端接入，不单独形成彼此隔离的完整路径。
8. ABC 相别优化：在共享低压网络确定后，对所有单相用户执行 `A/B/C` 相别分配。优化目标是降低台区出口及各共享线路段的 `A/B/C` 累计负荷不平衡；初始分配按视在容量从大到小贪心分配到该用户上游路径累计负荷最轻相，再通过局部交换、支路内调整及相邻支路协调降低不平衡。
9. 电气校核与修复：依据径向网络结构、共享线路段拓扑和 `assigned_phase` 自下而上汇总每条线路段的 `A/B/C` 负荷、零线电流、不平衡率及末端压降。若出现压降超限、零线电流过大或相间不平衡超限，触发相别重分配、局部换接、局部增杆、局部路径替换或共享主干调整，优先修复线路段级别的三相失衡。
...
- `data/outputs/plots/optimized_plan_2d.png`：2D 静态图，叠加地形、树林、水区、禁区、用户、配变、杆塔、高压线、低压共享主干/分支线和接户线。
...
用户按 `A/B/C` 相别使用固定颜色，接户线按接入相着色，主干线标注 `ABCN`。hover 信息包括用户负荷、功率因数、接入相、高程、挂接杆塔、线路平面长度、三维长度、高差、下游共享用户数、下游三相负荷、压降和净距状态。
...
- 共享接入：在存在空间相近用户的场景下，允许多个用户拥有相同 `attached_pole_id` 或共享同一 `lv_main/lv_branch` 路径，结果不应为每户单独生成一条完整主干线。
- 路径后插杆：沿路径布置杆塔后，相邻杆塔平面距离不得超过 50 m。
- 三维边计算：`length_3d >= horizontal_length`，高差、坡度、DTM 剖面采样和净距校核正确。
- 线路负荷汇总：对每条 `lv_main/lv_branch` 正确统计 `downstream_user_count` 与 `load_a_kva/load_b_kva/load_c_kva`，可反映多个用户共享后的累计负荷。
- ABC 相别分配：所有单相用户都有 `assigned_phase in {A, B, C}`，且台区出口及各共享低压线路段三相负荷不平衡率低于配置阈值。
- 可视化输出：2D 静态图、3D 静态图和 3D 动态图均生成。

## Assumptions

本阶段固定单台 630 kVA 配变；后续再扩展为多台、多容量型号自动选择。默认低压线路采用充分共享结构，用户作为终端挂接在共享杆塔或线路上，ABC 三相平衡指共享线路分段的累计相负荷平衡。上级高压电源点由配置给定，若未给定高程则从 `DTM` 采样。初版导线弧垂和杆塔结构只做简化净距校核，更精细的力学弧垂、转角杆、耐张杆和跨越规范作为后续扩展。当前方案为规划辅助工具，正式工程设计仍需按现行标准、当地供电公司要求和实际导线、杆塔型号复核。
